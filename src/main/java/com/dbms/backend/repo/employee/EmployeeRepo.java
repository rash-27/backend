...
public class EmployeeRepo {
    ...
    public List<EmployeeDetails> getEmployeeDetails() {
        try{
            ...
            return jdbcTemplate.query(sql, new EmployeeDetailsRowMapper());
        }catch (Exception e) {
    ...
    public void addEmployee(EmployeeDetails employeeDetails) {
        try{
            String sql = "INSERT INTO employee (name, role, address, join_date, salary) VALUES (?, ?, ?, ?, ?)";
            jdbcTemplate.update(sql, employeeDetails.name(), employeeDetails.role(), employeeDetails.address(), employeeDetails.join_date(), employeeDetails.salary());
        }catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    public Optional<EmployeeCompleteDetails> getEmployeeDetailsById(int id){
        try{
            ...
            EmployeeDetails employeeDetails = jdbcTemplate.query(sql, new EmployeeDetailsRowMapper(), id).stream().findFirst().orElse(null);
    ...
    public void updateEmployeeDetailsById(int id, EmployeeDetails employeeDetails) {
        try{
            String sql = "UPDATE employee SET name = ?, role = ?, address = ?, join_date = ?, salary = ? WHERE id = ?";
            jdbcTemplate.update(sql, employeeDetails.name(), employeeDetails.role(), employeeDetails.address(), employeeDetails.join_date(), employeeDetails.salary(), id);
        }catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
